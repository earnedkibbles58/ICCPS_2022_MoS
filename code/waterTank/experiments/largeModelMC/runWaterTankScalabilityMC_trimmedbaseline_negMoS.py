...
def main():
    ...
    for init_wl1 in init_wls:
        ...
        for init_wl2 in init_wls:
            ...
            WATER_TANK_BASELINE_FOLDER = "../../../../models/waterTank/trimmedBaselineMC_negMoS/if" + str(inflow) + "_of" + str(outflow) + "_deltawl" + str(deltawl) + "/"
            MODEL_FILE = WATER_TANK_BASELINE_FOLDER + 'waterTankBaselineMC_negMoS' + str(numSteps) + 'Steps.prism'
    ...
    # Save results
    RESULTS_FOLDER = "../../../../results/waterTank/largeModel_MC/trimmedBaseline_MC_negMoS/if" + str(inflow) + "_of" + str(outflow) + "_deltawl" + str(deltawl) + "_numSteps" + str(numSteps) + "/"
    try:
        os.makedirs(RESULTS_FOLDER)
    except OSError as e:
        pass

    np.save(RESULTS_FOLDER + "/runtimesWaterTankTrimmedBaseline_negMoS.npy",run_times)
    np.save(RESULTS_FOLDER + "/safetyProbsWaterTankTrimmedBaseline_negMoS.npy",safety_probs)
