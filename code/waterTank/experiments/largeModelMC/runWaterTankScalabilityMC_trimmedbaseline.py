...
def main():
    ...
    for init_wl1 in init_wls:
        ...
        for init_wl2 in init_wls:
            ...
            MODEL_FILE = WATER_TANK_BASELINE_FOLDER + 'waterTankBaselineMC' + str(numSteps) + 'Steps.prism'
    ...
    # Save results
    RESULTS_FOLDER = "../../../../results/waterTank/largeModel_MC/trimmedBaseline_MC/if" + str(inflow) + "_of" + str(outflow) + "_deltawl" + str(deltawl) + "_numSteps" + str(numSteps) + "/"
    try:
        os.makedirs(RESULTS_FOLDER)
    except OSError as e:
        pass

    np.save(RESULTS_FOLDER + "/runtimesWaterTankTrimmedBaseline.npy",run_times)
    np.save(RESULTS_FOLDER + "/safetyProbsWaterTankTrimmedBaseline.npy",safety_probs)
