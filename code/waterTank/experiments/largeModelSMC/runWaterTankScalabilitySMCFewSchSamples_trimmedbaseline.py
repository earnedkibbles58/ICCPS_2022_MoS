...
def main():
    ...
    for init_wl1 in init_wls:
        ...
        for init_wl2 in init_wls:
            ...
            for i in range(iter_per_init):
                ...
                proc = subprocess.run([MODEST_PATH, 'simulate',MODEL_FILE,'--lss','Sequential','-L','1','-C','0.8','-W','0.05','-E',"wlidInit1=" + str(init_wlid1) + ",wlidInit2=" + str(init_wlid2) + ",initContAction1=0,initContAction2=0",'-J','40'], stdout=PIPE, stderr=PIPE)
    ...
    # Save results
    RESULTS_FOLDER = "../../../../results/waterTank/largeModel_SMC/trimmedBaselineSMC_fewSchSamples/if" + str(inflow) + "_of" + str(outflow) + "_deltawl" + str(deltawl) + "_numSteps" + str(numSteps) + "/"
    try:
