function [scrubbedNextds,scrubbedNextvs] = scrubTransitionsBestCase(nextds,nextvs,deltad,deltav,safetyThresh)
%UNTITLED Summary of this function goes here
%   Detailed explanation goes here

crashInd = ceil((safetyThresh-0.00000001)/deltad);
...
[nextvids,I] = sort(nextvids,'ascend');
nextdids = nextdids(I);
nextds = nextds(I);
nextvs = nextvs(I);

[nextdids,I] = sort(nextdids,'descend');
nextvids = nextvids(I);
nextds = nextds(I);
nextvs = nextvs(I);            
...
n = length(nextdids);
if n >0
    if nextdids(n)<=crashInd
        scrubbedNextds = [nextds(n)];
        scrubbedNextvs = [nextvs(n)];
        return
    end
end
%%
