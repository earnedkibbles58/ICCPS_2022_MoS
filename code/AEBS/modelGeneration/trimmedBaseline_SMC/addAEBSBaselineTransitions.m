...
for i=1:latticeSize(1)
    for j=1:latticeSize(2)
        ...
        d = tempCell.d;
        v = tempCell.v;
        if(tempCell.did==11 && tempCell.vid==6)
            aegre=3;
        end
        newMapKey = N;
        clear newMapValue
        for K=0:1:N
            ...
            [nextds,nextvs] = computeAEBSBaselineTransition(d,v,N,K,deltad,deltav,B1,B2,fmu,amax,Thd,xThresh,TTCThresh,freq,safetyThresh,numDPerCell,numVPerCell,tempCell.did,tempCell.vid);

%             nextds
%             nextvs
            if(rounding==1)
                [nextds,nextvs] = scrubTransitionsWorstCase_SMC(nextds,nextvs,deltad,deltav);
            end
            latticeTransitions = [];
%             nextds
%             nextvs
