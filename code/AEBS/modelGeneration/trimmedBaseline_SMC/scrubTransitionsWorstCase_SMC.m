function [scrubbedNextds,scrubbedNextvs] = scrubTransitionsWorstCase_SMC(nextds,nextvs,deltad,deltav)
...
if isWorseThanAll(nextdids(1),nextvids(1),nextdids,nextvids)
    scrubbedNextds = [scrubbedNextds; nextds(1)];
    scrubbedNextvs = [scrubbedNextvs; nextvs(1)];

    scrubbedNextdids = [scrubbedNextdids; nextdids(1)];
    scrubbedNextvids = [scrubbedNextvids; nextvids(1)];
...
function worseThanAll = isWorseThanAll(nextdid,nextvid,nextdids,nextvids)
    worseThanAll = 1;
    for j = 1:length(nextdids)
        if nextdid > nextdids(j) || nextvid < nextvids(j)
            worseThanAll = 0;
            return
        end
    end
