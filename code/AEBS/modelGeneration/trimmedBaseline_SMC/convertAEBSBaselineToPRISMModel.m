function [] = convertLatticeToPRISMModelScalability(lattice, maxN, fileName,deltad,deltav,amax,trimmed,safetyThresh,H,L)
...
safetyThreshInd = ceil((safetyThresh-0.00000001)/deltad); %% FIXME: change from floor to ceil
...
%         end
        ...
        for k = 1:length(currMapKeys)
            ...
            allDidNextAre1=1; % change to be set to 1 if ANY didnext=1
            for l = 1:length(val)
                nextPointsTrans = val{l};
                for m = 1:size(nextPointsTrans,1)
                    nextPointdid = nextPointsTrans(m,1);
...
%% copy relevant prism model file from perception folder here

perceptionFolder = '../perception/models/H' + string(H) + 'L' + string(L) + '/';
perceptionFile = 'distDisc' + string(deltad) + '.txt';
perceptionModel = perceptionFolder + perceptionFile;

perstr = fileread(perceptionModel);
fprintf(fid, '%s',perstr);
