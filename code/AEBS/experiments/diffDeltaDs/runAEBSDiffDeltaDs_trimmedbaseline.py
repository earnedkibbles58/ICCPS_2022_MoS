...
def main():
    ...
    for delta_d in delta_ds:
        ...
        AEBS_BASELINE_FOLDER = "../../../../models/AEBS/trimmedBaseline_MC_diffDeltads/H" + str(H) + "L" + str(L) + "/N_" + str(N) + "/dist_" + str(init_dist) + "_vel_" + str(init_vel) + "_deltad_" + str(delta_d) + "_deltav_" + str(delta_v) + "/"
    ...
    # Save results
    RESULTS_FOLDER = '../../../../results/AEBS/diffDeltaDs_MC/trimmedBaseline_MC/'
    try:
        os.makedirs(RESULTS_FOLDER)
    except OSError as e:
        pass

    np.save(RESULTS_FOLDER + "/runtimesAEBSBaselineMC.npy",run_times)
    np.save(RESULTS_FOLDER + "/safetyProbsAEBSBaselineMC.npy",safety_probs)
