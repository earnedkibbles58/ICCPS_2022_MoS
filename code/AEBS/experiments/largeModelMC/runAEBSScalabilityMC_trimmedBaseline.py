...
def main():
    ...
    for init_dist in init_dists:
        ...
        for init_vel in init_vels:
            ...
            AEBS_BASELINE_FOLDER = "../../../../models/AEBS/trimmedBaseline_MC/H" + str(H) + "L" + str(L) + "/N_" + str(N) + "/dist_" + str(init_dist) + "_vel_" + str(init_vel) + "_deltad_" + str(delta_d) + "_deltav_" + str(delta_v) + "/"
            MODEL_FILE = AEBS_BASELINE_FOLDER + 'AEBSbaseline_withMCRounding.prism'
    ...
    # Save results
    RESULTS_FOLDER = '../../../../results/AEBS/largeModel_MC/trimmedBaseline_MC/'
    try:
        os.makedirs(RESULTS_FOLDER)
    except OSError as e:
        pass

    np.save(RESULTS_FOLDER + "/runtimesAEBSTrimmedBaselineMC.npy",run_times)
    np.save(RESULTS_FOLDER + "/safetyProbsAEBSTrimmedBaselineMC.npy",safety_probs)
